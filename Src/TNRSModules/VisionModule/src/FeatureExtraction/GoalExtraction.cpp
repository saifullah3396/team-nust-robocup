...
GoalExtraction::classifyPosts(vector<ScannedRegionPtr>& verGoalRegions)
{
  ...
  for (int i = 0; i < verGoalRegions.size(); ++i) {
    auto gr = verGoalRegions[i];
    Rect r = gr->rect;
    ...
    if (!goalPosts.empty()) {
      ...
      if (goalInfo.ours == 1) {
        for (int i = 0; i < goalPosts.size(); ++i) {
          ...
          //PRINT("Goal post: " << goalPosts[i]->world << endl)
        }
...
GoalExtraction::processImage()
{
  ...
  //PRINT("Scanning goal posts...")
  if (!scanForPosts(verGoalLines)) return;
#ifdef DEBUG_BUILD
  auto tEnd = high_resolution_clock::now();
  duration<double> timeSpan = tEnd - tStart;
  //PRINT("GoalExtraction.ScanForPosts.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  vector<ScannedRegionPtr> verGoalRegions;
  ////PRINT("Finding goal regions...")
  findRegions(verGoalRegions, verGoalLines, 8, 8, false);
#ifdef DEBUG_BUILD
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //PRINT("GoalExtraction.FindRegions.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  ////PRINT("Classifying goal regions...")
  classifyPosts(verGoalRegions);
#ifdef DEBUG_BUILD
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //PRINT("GoalExtraction.ClassifyPosts.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  ////PRINT("Finding best posts...")
  findBestPosts();
#ifdef DEBUG_BUILD
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //PRINT("GoalExtraction.FindBestPosts.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  updateGoalInfo();
#ifdef DEBUG_BUILD
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //PRINT("GoalExtraction.UpdateGoalInfo.Time: " << timeSpan.count() << " seconds.")
  //tStart = high_resolution_clock::now();
#endif
  //cout << "goalPosts.size(): " << goalPosts.size() << endl;
  //auto tEnd = high_resolution_clock::now();
  //duration<double> timeSpan = tEnd - tStart; 
  //PRINT("GoalExtraction.Update.Time: " << timeSpan.count() << " seconds.")
  /*#ifdef DEBUG_BUILD  
