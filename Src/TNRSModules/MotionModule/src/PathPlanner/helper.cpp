...
namespace PathPlannerSpace
{
  ...
  {
    double d = distanceMap.distanceMapAt(x, y);
    ...
    if (d >= r_o) return false;
    else if (accuracy == 0) return false;

    const double h_half = height / 2.0;
    const double w_half = width / 2.0;
    const double r_i = std::min(w_half, h_half);

    if (d <= r_i) return true;
    else if (accuracy == 1) return true;
    ...
    const std::vector<std::pair<int, int> >& edges)
  {
    ...
    for (unsigned int i = 0; i < edges.size() - 1; ++i) {
      if ((edges[i].second <= y && edges[i + 1].second > y) || (edges[i].second > y && edges[i + 1].second <= y)) {
        float vt =
          (float) (y - edges[i].second) / (edges[i + 1].second - edges[i].second);
