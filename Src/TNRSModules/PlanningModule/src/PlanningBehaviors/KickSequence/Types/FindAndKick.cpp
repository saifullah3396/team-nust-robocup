...
{
  PRINT("FindAndKick.initiate()...")
  inBehavior = true;
}

void FindAndKick::update()
{
  //PRINT("FindAndKick.update()...")
...
{
  PRINT("FindAndKick.headTapWaitAction()...")
...
{
  PRINT("FindAndKick.startupAction()...")
...
{
  //PRINT("FindAndKick.searchBallAction()...")
  bool headTapped =
    IVAR(vector<float>, PlanningModule::touchSensors)[HEAD_TOUCH_MIDDLE] > 0.f;
  if(headTapped) {
    killMotionBehavior();
