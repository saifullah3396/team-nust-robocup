...
RobocupSetup::update()
{
  ...
  //
  //PRINT("In RobocupSetup Gameplay Loop...")
...
{
  PRINT("Executing RobocupSetup.startupAction()...")
  if (posture == PostureState::STAND && 
      stiffness == StiffnessState::ROBOCUP) 
  {
    PRINT("Setting RobocupSetup Gameplay Startup...")
...
{
  PRINT("Executing RobocupSetup.cfgHandlingAction()...")
  //PRINT("Setting RobocupSetup Configuration...")
...
{
  PRINT("Executing RobocupSetup.readySequenceAction()...")
  auto& localized = IVAR(bool, PlanningModule::robotLocalized);
  if (!localized) {
    PRINT("goalInfo.found: " << IVAR(GoalInfo, PlanningModule::goalInfo).found)
    PRINT("goalInfo.ours: " << IVAR(GoalInfo, PlanningModule::goalInfo).ours)
    PRINT(
    ...
    if (lastMBFinished) {
      PRINT("Setting Find Goal Behavior...")
      auto mConfig =
...
{
  PRINT("Executing RobocupSetup.getInPositionsAction()...")
  auto& gameData = IVAR(RoboCupGameControlData, PlanningModule::gameData);
  ...
  if ((unsigned) gameData.state == STATE_SET) {
    ...
    auto& localized = IVAR(bool, PlanningModule::robotLocalized);
    if (!localized) {
      PRINT("Robot not localized. Trying to scan and get localized...")
      OVAR(bool, PlanningModule::localizeWithLastKnown) = true;
      if (lastMBFinished) {
        auto mConfig =
          boost::make_shared <HeadTargetSearchConfig> (
      ...
      if (lastMBFinished && lastMotionRequest->id == (unsigned) MBIds::MOVEMENT) {
        PRINT("Walk finished. Resetting localizer...")
        reqRelocalization();
        OVAR(bool, PlanningModule::localizeWithLastKnown) = true;
      } else if (lastMBFinished) {
        PRINT("Setting walk with target...")
...
{
  PRINT("Executing RobocupSetup.setSequenceAction()...")
