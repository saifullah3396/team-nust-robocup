...
GoalKeeper::initiate()
{
  PRINT("GoalKeeper behavior initiated...")
  inBehavior = true;
}
...
GoalKeeper::update()
{
  //PRINT("Executing GoalKeeper.update()...")
  // IMPORTANT BUG
...
{
  //PRINT("Executing GoalKeeper.goalKeeperAction()...")
  //PRINT("BehaviorState: " << behaviorState)
...
{
  PRINT("Executing GoalKeeper.correctPostureAction()...")
...
{
  PRINT("Exeucting GoalKeeper.findItselfAction()")
...
{
  //PRINT("Exeucting GoalKeeper.searchBallAction()")
  if (shouldSearchBall()) {
    if (lastMBFinished) {
...
{
  PRINT("Exeucting GoalKeeper.saveGoalAction()")
  auto& tbInfo = IVAR(TeamBallInfo, PlanningModule::teamBallInfo);
  auto ballWorld = tbInfo.posWorld;
  if (ballWorld.x >= -4.5 && ballWorld.x <= -3.75 && ballWorld.y >= -1.15 && ballWorld.y <= 1.15 && shouldPlayBall()) {
    ...
    if (inter.x >= -4.75 && inter.x <= -4.25 && inter.y >= -0.75 && inter.y <= 0.75) {
      ...
      if (sqrtC < 0) { // Ball will stop before reaching the goal
        PRINT("Ball will stop before reaching the goal...")
...
{
  PRINT("Exeucting GoalKeeper.interceptBallAction()")
...
{
  PRINT("Exeucting GoalKeeper.diveAction()")
...
GoalKeeper::diveFollowUpAction()
{
  ...
  //{
  PRINT("Executing GoalKeeper.diveFollowUpAction()...")
