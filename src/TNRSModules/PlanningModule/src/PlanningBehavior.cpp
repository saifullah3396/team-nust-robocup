...
{
  LOG_INFO("Posture: " + DataUtils::varToString((int)posture))
  LOG_INFO("Stiffness: " + DataUtils::varToString((int)stiffness))
  LOG_INFO("Posture: " + DataUtils::varToString((int)desPosture))
  LOG_INFO("Stiffness: " + DataUtils::varToString((int)desStiffness))
  ...
  if (!waitForUnpenalise) {
    if (!penaliseMotion) {
      ...
      if (posture == PostureState::STAND && stiffness == StiffnessState::ROBOCUP) {
        ...
        if (lastMBFinished) {
          LOG_INFO("Setting posture to sit at startup.")
