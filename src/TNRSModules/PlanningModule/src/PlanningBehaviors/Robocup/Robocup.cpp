...
{
  LOG_INFO("Executing Robocup.fallRecoveryAction()...")
  ...
  if (fallen) {
    ...
    if (posture != PostureState::STAND) {
      if (lastMBFinished) {
        LOG_INFO("Setting posture to stand at fallen.")
        auto pConfig = 
...
Robocup::getupBack()
{
  LOG_INFO("Executing Robocup.getupBack()...")
  //LOG_INFO("Posture: " << (int)posture)
  //LOG_INFO("Stiffness: " << (int)stiffness)
  if (!readyToGetup) {
    //LOG_INFO("Getting ready to get up...")
    if (stiffness != StiffnessState::GETUP) {
      //LOG_INFO("lastSBFinished: " << (int)lastSBFinished)
      if (lastSBFinished) {
        //LOG_INFO("Setting StiffnessState to GETUP...")
        auto sConfig =
    ...
    if (lastMBFinished) {
      //LOG_INFO("Starting Getup Sequence from Back...")
      auto getupConfig =
...
Robocup::getupFront()
{
  LOG_INFO("Executing Robocup.getupFront()...")
  //LOG_INFO("Posture: " << (int)posture)
  //LOG_INFO("Stiffness: " << (int)stiffness)
  if (!readyToGetup) {
    //LOG_INFO("Getting ready to get up...")
    ...
    if (lastMBFinished) {
      //LOG_INFO("Starting Getup Sequence from Front...")
      auto getupConfig =
