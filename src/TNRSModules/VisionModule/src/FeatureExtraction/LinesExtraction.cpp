...
LinesExtraction::findCircle(Circle& circleOutput, vector<Point2f>& circlePoints)
{
  ...
  //LOG_INFO("LinesExtraction.CircleTime3.Update.Time: " << timeSpan.count() << " seconds.")
...
LinesExtraction::processImage()
{
  ...
  auto tEnd = high_resolution_clock::now();
  duration<double> timeSpan = tEnd - tStart;
  //LOG_INFO("LinesExtraction.ScanForEdges.Time: " << timeSpan.count() << " seconds.")
  ...
  //LOG_INFO("LinesExtraction.findLinesFromEdges.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  ...
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //LOG_INFO("LinesExtraction.filterLines.Time: " << timeSpan.count() << " seconds.")
  //! Draw world lines
  /*for (size_t i = 0; i < worldLines.size(); ++i) {
   if (!worldLines[i])
   continue;
  ...
  tEnd = high_resolution_clock::now();
  timeSpan = tEnd - tStart;
  //LOG_INFO("LinesExtraction.findFeatures.Time: " << timeSpan.count() << " seconds.")
  tStart = high_resolution_clock::now();
#endif
  ...
  if (findCircle(circleOutput, circlePoints)) {
    ...
    tEnd = high_resolution_clock::now();
    timeSpan = tEnd - tStart;
    //LOG_INFO("LinesExtraction.findCircle.Update.Time: " << timeSpan.count() << " seconds.")
  ...
  timeSpan = tEnd - tStart;
#endif  
  //LOG_INFO("LinesExtraction.addLineLandmarks.Time: " << timeSpan.count() << " seconds.")
  //imshow("ol", olImage);
  //waitKey(0);
}
