...
Eigen::Matrix<Scalar, Dynamic, 1> TaskIkSolver<Scalar>::step()
{
  ...
  for (size_t i = 0; i < nDof; ++i) {
    ...
    maxV[i] = min(joint->maxVelocity, maxDiffV);
    minV[i] = max(-joint->maxVelocity, minDiffV);
  }
...
Eigen::Matrix<Scalar, Dynamic, 1> TaskIkSolver<Scalar>::solve()
{
  ...
  for (size_t i = 0; i < maxIterations; ++i) {
    ...
    prevCost = cost;
    cost = computeCost();
    LOG_INFO("Cost: " + DataUtils::varToString(cost))
    ...
    //LOG_INFO("Iter Time: " << timeSpan.count() << "seconds.");
  }
  LOG_INFO("joints: ")
  for (size_t i = 0; i < NUM_JOINTS; ++i) {
    LOG_INFO(DataUtils::varToString(joints[i] * 180 / M_PI))
  }
  return joints;
}
