  ...
  try {
    ...
    if (reqVel.x != -1.f || target.x != -1.f) {
      ...
      if (!setEndEffectorXY(targetAngle)) {
        LOG_INFO("Unable to set end effector for kick.")
        return false;
      }
      //cout << "endEffector:\n" << endEffector << endl;
      return true;
...
JSOImpKick::findBestEEAndImpactPose()
{
  ...
  impactPose(0, 3) = ballPosition[0] - ballToTargetUnit[0] * ballRadius;
  impactPose(1, 3) = ballPosition[1] - ballToTargetUnit[1] * ballRadius;
