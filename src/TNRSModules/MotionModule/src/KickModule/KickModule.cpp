...
KickModule<Scalar>::initiate()
{
  ...
  LOG_INFO("KickModule type: " << (unsigned)type)
...
KickModule<Scalar>::update()
{
  ...
  if (getBehaviorCast()->ballTrack) {
    //LOG_INFO("KickModule.htModuleUpdate()")
    htModule->update();
  }

  if (behaviorState == preKickPostureInit) {
    //LOG_INFO("KickModule.preKickPostureInit()")
    ...
    //LOG_INFO("KickModule.preKickPostureExec()")
    ...
    //LOG_INFO("KickModule.kickExecution()")
    ...
    //LOG_INFO("KickModule.postKickInit()")
    ...
    //LOG_INFO("KickModule.postKickExec()")
    if (postureModule->getState()) postureModule->update();
...
KickModule<Scalar>::setupFixedVelocityKick()
{
  ...
  if (ball.x != -1.f && reqVel.x != -1.f) {
    ...
    LOG_INFO("Required kick parameters 'ball' & 'reqVel' not defined")
    return false;
  }
}
...
KickModule<Scalar>::setupVMBasedKick()
{
  ...
  if (ball.x != -1.f && targetInField.x != -1.f) {
    ...
    LOG_INFO("Required kick parameters 'ball' & 'target' not defined")
    return false;
  }
}*/
/*
...
{
  LOG_INFO("KickModule.setupBallTracking()")
  if (getBehaviorCast()->ballTrack) {
...
KickModule<Scalar>::defineTrajectory()
{
  ...
  if (getBehaviorCast()->type == MBKickTypes::FIXED_VELOCITY) {
    ...
    LOG_INFO("Performing first trajectory optimization...")
...
//{
 ...
 /* LOG_INFO("Starting VM kick design")
  ...
  LOG_INFO("Initial configurations set")
