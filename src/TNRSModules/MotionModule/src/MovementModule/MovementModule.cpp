...
{
  LOG_INFO("Initiating MovementModule")
  ...
  if (setGoal(goal)) {
    LOG_INFO("MovementModule.setGoal() successful. Continuing...")
  } else {
  ...
  LOG_INFO("MovementModule initiation successful...")
}

template <typename Scalar>
void MovementModule<Scalar>::reinitiate(const BehaviorConfigPtr& cfg)
{
  this->config = cfg;
  if (getBehaviorCast()->htConfig)
    this->setupChildRequest(getBehaviorCast()->htConfig, true);
...
{
  LOG_INFO("Updating MovementModule")
...
void MovementModule<Scalar>::setupNaoQiWalk()
{
  ...
  prevStep = -1;
  /**
   * FIXME: Temporarily removed due to addition of MotionGenerator
   * Reroute naoqi motion proxy usage through MotionGenerator
...
void MovementModule<Scalar>::executeWalk()
{
  ...
  //LOG_INFO("Start walking towards the goal.")
  if (!inWalk) {
    setupNaoQiWalk();
  } else {
